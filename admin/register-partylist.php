...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty(trim($_POST["name"]))) {
    $name_error = "Please enter a party list name.";
  } else {
  ...
  if (empty($name_error)) {
    $insertPartyList = "INSERT INTO party_lists (name) VALUES (?)";

    if ($stmt = mysqli_prepare($connection, $insertPartyList)) {
      mysqli_stmt_bind_param($stmt, "s", $param_name);
...
<html lang="en">
  ...
  <body>
    <header>
      ...
      <div id="mySidenav" class="sidenav">
        ...
        <li class="t-li t-li-1"><a href="./" class="t-nav-links">View Results</a></li>
          ...
          <li class="t-li"><a href="register-partylist.php" class="t-nav-links active">Register Partylist</a></li>
    ...
    <!-- ADD PARTY-LIST FORM -->
    <div class="form-box">
      <h2 class="form-h">Register Partylist</h2>
      <?php 
      ...
      <div class="content">
        <form autocomplete="off" action="" method="POST">
          <div class="input-box">
            ...
            <input autofocus type="text" name="name" class="form-input" value="<?php
            if (empty($add_success)) {
              if (!empty($name)) {
                echo $name;
              }
            ...
            ?>">
            <span class="error"><?php if (!empty($name_error)) echo $name_error ?><span>
          </div>
