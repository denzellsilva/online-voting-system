    ...
    <header>
      ...
      <div id="mySidenav" class="sidenav">
        ...
        <ul>
          ...
          <li class="t-li"><a href="view-positions.php" class="t-nav-links active">View Positions</a></li>
        </ul>
      </div>
      <div id="mySidenav" class="sidenav">
        <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
    ...
    <main>
      <div class="table-container partylist-table">
        <table>
          ...
          <thead>
            <tr>
                <th class="th-position">Position Name</th>
                <th>Maximum Candidate per Party</th>
            ...
            <?php
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result)) {
                        echo '<tr><td>' .$row['position'].'</td><td>' .$row['maximum'].'</td>
                        <td><a href="edit-position.php?id='.$row["id"].'">
                            <button>Edit</button>
                  </a>
                  </td>
                  <td><a onClick="return confirm(\'Proceed to Delete?\');" href="delete-position.php?id=' . $row["id"] . '" title="Delete">
                      <button>Delete</button>
                  </a></td></tr>';
              }
            } else {
              echo "<tr><td colspan='4'><center>Add a position first.</center></td></tr>";
          }
            ?>
          </tbody>
        </table>
